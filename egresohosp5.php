...
$subtitulo="";
	if(isset($_POST["operacion"])){	//nivel3
		if($_POST["aceptar"]!="Descartar"){
			...
			case 'E':
			$sql="UPDATE adm_hospitalario set fegreso_hosp='".$_POST["fegreso"]."',hegreso_hosp='".$_POST["hegreso"]."',estado_salida='".$_POST["esalida"]."',via_salida='".$_POST["viasalida"]."',medsalida='".$_SESSION["AUT"]["nombre"]."',estado_adm_hosp='Parcial' WHERE id_adm_hosp='".$_POST["idadmhosp"]."'";
			$subtitulo="Realizado";
			break;
			case 'X':
	...
	<section class="panel panel-default">
		<section class="panel-body">
			<article class="col-xs-3">
				...
				<input type="text" name="name" class="form-control" value="<?php echo $fila["tdoc_pac"].':  '.$fila["doc_pac"];?>">
				<input type="hidden" name="idadmhosp" class="form-control" value="<?php echo $fila["id_adm_hosp"];?>">
			</article>
			<article class="col-xs-3">
				<label for="">Nombre Completo:</label>
				<input type="text" name="name" class="form-control" value="<?php echo $fila["nom1"].' '.$fila["nom2"].' '.$fila["ape1"].' '.$fila["ape2"];?>">
			</article>
			<article class="col-xs-3">
				<label for="">Fecha y Hora INGRESO:</label>
				<input type="text" name="name" class="form-control" value="<?php echo $fila["fingreso_hosp"].' | '.$fila["hingreso_hosp"];?>">
			</article>
			<article class="col-xs-3">
