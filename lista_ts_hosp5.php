...
$subtitulo="";
	if(isset($_POST["operacion"])){	//nivel3
		if($_POST["aceptar"]!="Descartar"){
			...
			break;
			case 'TRAS':
				$sql="INSERT INTO traslados (id_adm_hosp, id_user, freg_traslado, hreg_traslado, fprog_envio, hprog_envio,origen, obs_envio, resp_envio, estado_traslado, medio_transporte)
				VALUES ('".$_POST["idadmhosp"]."','".$_SESSION["AUT"]["id_user"]."','".$_POST["freg"]."','".$_POST["hreg"]."','".$_POST["fprog"]."','".$_POST["hprog"]."','".$_POST["sedes"]."','".$_POST["obs_envio"]."','".$_SESSION["AUT"]["nombre"]."','Enviado','".$_POST["mediots"]."')";
...
if (isset($_GET["mante"])){					///nivel 2
    ...
    where b.id_adm_hosp ='".$_GET["idadmhosp"]."'" ;
			...
			$date=date('Y-m-d');
			$date1=date('H:i');
			$form1='formularios/valinitshosp.php';
      ...
      where b.id_adm_hosp ='".$_GET["idadmhosp"]."'" ;
			...
			$date=date('Y-m-d');
			$date1=date('H:i');
			$form1='formularios/evotshosp.php';
      ...
      where b.id_adm_hosp ='".$_GET["idadmhosp"]."'" ;
			...
			$date=date('Y-m-d');
			$date1=date('H:i');
			$form1='formularios/ordtraslado.php';
	...
	<table class="table table-responsive">
		...
		if ($tabla=$bd1->sub_tuplas($sql)){
			...
			foreach ($tabla as $fila ) {
				...
				echo'<th class="text-center"><a href="'.PROGRAMA.'?opcion='.$_REQUEST["opcion"].'&mante=VALINI&idadmhosp='.$fila["id_adm_hosp"].'"><button type="button" class="btn btn-primary sombra_movil" ><span class="fa fa-plus-circle"></span></button></a></th>';
				...
				echo'<th class="text-center"><a href="'.PROGRAMA.'?opcion='.$_REQUEST["opcion"].'&mante=TRAS&idadmhosp='.$fila["id_adm_hosp"].'"><button type="button" class="btn btn-success sombra_movil" ><span class="fa fa-wheelchair-alt"></span></button></a></th>';
				echo "</tr>\n";
			}
		}
	}
	if (isset($_REQUEST["nom"])){
		...
		if ($tabla=$bd1->sub_tuplas($sql)){
			...
			foreach ($tabla as $fila ) {
				...
				echo'<th class="text-center"><a href="'.PROGRAMA.'?opcion='.$_REQUEST["opcion"].'&mante=VALINI&idadmhosp='.$fila["id_adm_hosp"].'"><button type="button" class="btn btn-primary sombra_movil" ><span class="fa fa-plus-circle"></span></button></a></th>';
				echo'<th class="text-center"><a href="'.PROGRAMA.'?opcion='.$_REQUEST["opcion"].'&mante=EVO&idadmhosp='.$fila["id_adm_hosp"].'"><button type="button" class="btn btn-primary sombra_movil" ><span class="fa fa-stethoscope"></span></button></a></th>';
				echo'<th class="text-center"><a href="'.PROGRAMA.'?opcion='.$_REQUEST["opcion"].'&mante=TRAS&idadmhosp='.$fila["id_adm_hosp"].'"><button type="button" class="btn btn-success sombra_movil" ><span class="fa fa-rocket"></span></button></a></th>';
				echo "</tr>\n";
			}
		}
	}
