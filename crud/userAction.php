...
if(isset($_POST['action_type']) && !empty($_POST['action_type'])){
    if($_POST['action_type'] == 'data'){
        ...
        $users = $db->getRows($tblName,array('order_by'=>'freg_evofisio_reh DESC'));
        if(!empty($users)){
            $count = 0;
            foreach($users as $user): $count++;
                echo '<tr>';
                echo '<td>#'.$count.'</td>';
                echo '<td>'.$user['id_evofisio_reh'].'</td>';
                echo '<td>'.$user['freg_evofisio_reh'].'</td>';
                echo '<td>'.$user['hreg_evofisio_reh'].'</td>';
                echo '<td>'.$user['evolucionfisio_reh'].'</td>';
