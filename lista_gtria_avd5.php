...
$subtitulo="";
	if(isset($_POST["operacion"])){	//nivel3
		if($_POST["aceptar"]!="Descartar"){
			...
			break;
			case 'EVO':
				$sql="INSERT INTO evo_gtria_inde (id_adm_hosp, id_user, freg_evo_gtria, hreg_evo_gtria, objetivo_gtria, subjetivo_gtria, analisis_evo_gtria, plan_tratamiento_gtria, dxp, tdxp, dx1, tdx1, dx2, tdx2, servicio, estado_evo_gtria) VALUES
				('".$_POST["idadmhosp"]."','".$_SESSION["AUT"]["id_user"]."','".$_POST["freg"]."','".$_POST["hreg"]."','".$_POST["objetivo"]."','".$_POST["subjetivo"]."','".$_POST["analisis"]."','".$_POST["plantratamiento"]."','".$_POST["dx"]."','".$_POST["tdx"]."','".$_POST["dx1"]."','".$_POST["tdx1"]."','".$_POST["dx2"]."','".$_POST["tdx2"]."','AVD','Realizada')";
				$subtitulo="Evolución Medica de Geriatria";
				$sub1='Demencias';
				$subtitulo1="Adicionado";
			break;
			case 'ADX':
...
if (isset($_GET["mante"])){					///nivel 2
    ...
    $sql="SELECT a.tdoc_pac,a.doc_pac,nom1,nom2,ape1,ape2,edad,fnacimiento,dir_pac,tel_pac,rh,email_pac,genero,lateralidad,religion,fotopac,
		...
		where b.id_adm_hosp ='".$_GET["idadmhosp"]."'" ;
			...
			$date1=date('H:i');
			$return=128;
			$form1='FormulariosINDE/val_inigtria_inde.php';
			$subtitulo='Valoracion Inicial Geriatria';
			break;
			case 'EVO':
      $sql="SELECT a.tdoc_pac,a.doc_pac,nom1,nom2,ape1,ape2,edad,fnacimiento,dir_pac,tel_pac,rh,email_pac,genero,lateralidad,religion,fotopac,
		...
		<script>
			function validar(){
				if (document.forms[0].hreg.value > document.forms[0].fecha.value){
					alert("Medico(a) especialista en Geriatria <?php echo $_SESSION["AUT"]["nombre"]?>, NO puede adelantar el tiempo.");
...
<section class="panel-heading"><h4>Programa AVD Especialidad Geriatria</h4></section>
	...
	<table class="table table-responsive">
		...
		if ($tabla=$bd1->sub_tuplas($sql)){
			...
			foreach ($tabla as $fila ) {
				...
				echo'<th class="text-center"><a href="'.PROGRAMA.'?opcion=133&idadmhosp='.$fila["id_adm_hosp"].'&servicio=AVD"><button type="button" class="btn btn-success sombra_movil" ><span class="fa fa-flask"></span></button></a></th>';
