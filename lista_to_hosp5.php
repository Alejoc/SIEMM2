...
$subtitulo="";
	if(isset($_POST["operacion"])){	//nivel3
		if($_POST["aceptar"]!="Descartar"){
			...
			break;
			case 'A':
				$sql="INSERT INTO evo_to (id_adm_hosp,id_user,freg_evoto,hreg_evoto,evolucion_to,estado_evoto,resp_evoto) VALUES
		...
		//echo $sql;
		if ($bd1->consulta($sql)){
			$subtitulo="Formato de evolucion $subtitulo con exito!";
...
if (isset($_GET["mante"])){					///nivel 2
	switch ($_GET["mante"]) {
		case 'E':
			...
			$atributo2='';
			$atributo3='';
			$subtitulo='Edicion de datos del VehÃ­culo';
			break;
			case 'X':
			$sql="";
			$color="red";
			...
			$subtitulo='Confirmacion para eliminar de datos del VehÃ­culo';
			break;
			case 'A':
...
<table class="table table-bordered">
	...
	$doc=$_REQUEST["nom"];
	$sql="SELECT p.id_paciente,tdoc_pac,doc_pac,nom1,nom2,ape1,ape2,fotopac,a.id_adm_hosp,fingreso_hosp,hingreso_hosp,s.nom_sedes FROM pacientes p LEFT JOIN adm_hospitalario a on p.id_paciente=a.id_paciente LEFT JOIN sedes_ips s on a.id_sedes_ips=s.id_sedes_ips  WHERE p.nom1 LIKE '%".$doc."%' and a.estado_adm_hosp='Activo'";
