...
class MYPDF extends TCPDF {
	...
	public function ColoredTable($header,$data,$data1) {
		...
		$fill = 0;
		foreach($data as $row) {
			$this->multiCell(180, 6, $row[0].''.$row[1].' Profesional:'.$row[10].' Identificacion:'.$row[11].' RM:'.$row[12].' '.utf8_encode($row[13]), 1, 'J');
      $this->multicell(180, 6, utf8_encode($row[2]),1, 0, 'J');
      $this->multicell(180, 6, utf8_encode($row[3].''.$row[4].''.$row[5].''.$row[6].''.$row[7]), 0, 'J');
