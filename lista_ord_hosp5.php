...
<div class="panel panel-default">
	<section class="panel-heading"><h4>Listado de ordenes en <?php echo $_REQUEST['servicio'];?></h4></section>
...
<table class="table table-responsive table-bordered">
	<tr>
		...
		<th id="th-estilo2">DIAGNOSTICO</th>
		<th id="th-estilo2">PROCEDIMIENTO</th>
		<th id="th-estilo2">OBSERVACION</th>
	...
	$sql="SELECT * FROM ord_med_hosp WHERE id_adm_hosp =".$idadmhosp;

if ($tabla=$bd1->sub_tuplas($sql)){
	foreach ($tabla as $fila ) {
		if ($fila['estado_ord_med_hosp']=='Solicitado') {
			echo"<tr>\n";
			echo'<td class="text-right alert-success">'.$fila["id_ord_med_hosp"].'</td>';
			echo'<td class="text-left alert-success">'.$fila["ts_ord_med_hosp"].'</td>';
			echo'<td class="text-left alert-success">'.$fila["freg_ord_med_hosp"].' | '.$fila["hreg_ord_med_hosp"].'</td>';
			echo'<td class="text-left alert-success">'.$fila["dx"].'</td>';
			echo'<td class="text-left alert-success">'.$fila["procedimiento"].'</td>';
			echo'<td class="text-left alert-success">'.$fila["obs_proc"].'</td>';
			echo'<td class="text-left alert-success">'.$fila["estado_ord_med_hosp"].'</td>';
			echo'<th class="text-center alert-success"><a href="rpt_ordenhosp.php?idordhosp='.$fila["id_ord_med_hosp"].'&idadmhosp='.$idadmhosp.'"><button type="button" class="btn btn-success" ><span class="fa fa-print"></span></button></a></th>';
	...
	?>
	<tr>
		<th colspan="11" class="text-center"><a href="<?php echo PROGRAMA.'?opcion='.$_REQUEST["opcion"].'&servicio='.$servicio.'&mante=A&idadmhosp='.$idadmhosp?>" align="center" ><button type="button" class="btn btn-primary" >Adicionar Orden de servicio</button>
		</a></th>
	<tr>
</table>
</div>
