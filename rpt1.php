...
i.descripuedad,
j.nom_eps,
k.id_evomed,'EVOLUCION MEDICA' tipo_evo,k.freg_evomed fecha_evo,k.hreg_evomed hora_evo, concat('Objetivo: ',k.objetivo, ' Subjetivo:',k.subjetivo, ' Analisis:' ,k.analisis_evomed, ' Plan tratamienti:' ,k.plan_tratamiento) evolucion,
...
from pacientes a left join adm_hospitalario b on a.id_paciente=b.id_paciente
      ...
      left join evo_psicologia m on (m.id_adm_hosp=b.id_adm_hosp)
      left join evo_to n on (n.id_adm_hosp=b.id_adm_hosp)
...
i.descripuedad,
j.nom_eps,
k.id_evo_psico,'EVOLUCION PSICOLOGIA' tipo_evo,k.freg_evo_psico fecha_evo, k.hreg_evo_psico hora_evo, concat(k.tipo_sesion,' ', k.obj_sesion,' ', k.actividades,' ', k.resultado,' ', k.obs_evo_psico) evolucion,
l.nombre,rm_profesional,l.especialidad espec_user,firma
from pacientes a left join adm_hospitalario b on a.id_paciente=b.id_paciente
      left join estado_civil c on (c.codestadoc = a.estadocivil)
      left join tusuario e on (e.codtusuario=b.tipo_usuario)
...
from pacientes a left join adm_hospitalario b on a.id_paciente=b.id_paciente
      ...
      left join uedad i on (i.coduedad=a.uedad)
      left join eps j on (j.id_eps=b.id_eps)
      left join nota_enfermeria k on (k.id_adm_hosp=b.id_adm_hosp)
      left join user l on (l.id_user=k.id_user)
...
from pacientes a left join adm_hospitalario b on a.id_paciente=b.id_paciente
      ...
      left join uedad i on (i.coduedad=a.uedad)
      left join eps j on (j.id_eps=b.id_eps)
      left join evo_ts k on (k.id_adm_hosp=b.id_adm_hosp)
      left join user l on (l.id_user=k.id_user)
...
i.descripuedad,
j.nom_eps,
k.id_val_nutri,'NUTRICION Y DIETETICA' tipo_evo, k.freg_nutri fecha_evo, k.hreg_nutri hora_evo, concat(k.motivo_nutri, k.val_nutricional, k.dx_nutricional, k.plan_nutricional )evolucion,
l.nombre,rm_profesional,l.especialidad espec_user,firma
from pacientes a left join adm_hospitalario b on a.id_paciente=b.id_paciente
      left join estado_civil c on (c.codestadoc = a.estadocivil)
      left join tusuario e on (e.codtusuario=b.tipo_usuario)
      ...
      left join uedad i on (i.coduedad=a.uedad)
      left join eps j on (j.id_eps=b.id_eps)
      left join val_nutricion k on (k.id_adm_hosp=b.id_adm_hosp)
      left join user l on (l.id_user=k.id_user)
